module.exports = (options, peers, heartbeats) => {
  ...
  return {
    ...
    send:(address, message) => {
      ...
      needle.get(address + `/message?message=${message}&address=${options.myAddress}`, function(error, response) {
    ...
    connect:address => {
      needle.get(address + `/connect?address=${options.myAddress}`, function(error, response) {
        ...
        if(response && response.statusCode == 200) heartbeats.update(address)
      })
    }
  }
}
